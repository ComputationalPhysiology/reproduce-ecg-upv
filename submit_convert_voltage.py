...
template = dedent(
    """#!/bin/bash
#SBATCH --job-name="convert-voltage-{sex}-{case}"
...
/home/henriknf/miniforge3/envs/fenicsx-v09/bin/python3 ${{ROOT}}/main_fenicsx.py convert-voltage -d ${{ROOT}}/hex-mesh -r ${{SCRATCH_DIRECTORY}}
...
def main():
    ...
    for sex in ["male", "female"]:
        for case in [c.name for c in Case]:
            ...
            if not (outdir / "voltage.mp4").exists():
                print(f"Skipping {outdir.name}: no voltage.mp4 file")
                continue
